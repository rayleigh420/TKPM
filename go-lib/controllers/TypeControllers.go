...
func InsertTypeByName(type_name string) (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	res := bson.M{}
	err := TypeCollections.FindOne(ctx, bson.M{"typename": type_name}).Decode(&res)
	count, _ := TypeCollections.CountDocuments(ctx, bson.D{{}})
	count++
	counts := strconv.Itoa(int(count))
	if err != nil {
		TypeCollections.InsertOne(ctx, models.BookTypeModel{
			Id:       primitive.NewObjectID(),
			TypeId:   counts,
			TypeName: type_name,
		},)
		return counts,nil
	}else{ return res["typeid"].(string), nil}
